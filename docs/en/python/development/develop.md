...
```bash
# Skip the MBR partition table, back up OneCloud U-Boot contents and compress
...
References: [gpioset command | Everyone Understands IoT (getiot.tech)](https://getiot.tech/zh/linux-command/gpioset)

??? tip "All GPIO pins on OneCloud 5.x Linux kernels"
    === "gpioinfo"
        ```
        gpiochip0 - 83 lines:
...
```bash
# Set the display background image
...
# Output capture card video to the HDMI display
...
Control code for the CUS_HID USB relay.
...
- [Ubuntu 20.04 LCUS_HID USB relay driver (CSDN)](https://blog.csdn.net/nibiewuxuanze/article/details/127069479)


### Reset USB Devices on Linux
...
After installation, reset a USB device with the following command. Replace Bus 001 Device 004 with your USB device's bus and device numbers:

```bash
sudo usbreset /dev/bus/usb/001/004
```
Or use one of the following commands, where `VID:PID` is the USB device ID from `lsusb`, or `bus:device` is the bus and device number:
