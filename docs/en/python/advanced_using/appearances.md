Static web content lives in `/usr/share/kvmd/web/` (source path: `/web/`).

### Icons

To replace the logo or other icons, replace files under `/usr/share/kvmd/web/share/` (source path: `/web/share/`). For example, `svg/logo.svg` is the app logo.

### Color Themes

To customize the UI theme, edit `/usr/share/kvmd/web/share/css/user.css` and add your CSS color theme.

Below are sample themes you can adjust as needed.
