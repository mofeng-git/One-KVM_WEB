...
电脑主板 9 Pin 中的 POWER LED+ 和 POWER LED- 分别连接光耦隔离模块输入端的正极和负极（有严格对应关系），光耦隔离模块输出端的 GND和 OUT 分别玩客云的电源状态灯焊盘和 GND 焊盘（无严格对应关系）。

电脑主板 9 Pin 中的 HDD LED+ 和 HDD LED- 分别连接光耦隔离模块输入端的正极和负极（有严格对应关系），光耦隔离模块输出端的 GND 和 OUT 分别玩客云的 HDD 状态灯焊盘和 GND 焊盘（无严格对应关系）。
