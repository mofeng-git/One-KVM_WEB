...
虚拟机 One-KVM 整合包为预装了 One-KVM Rust 的虚拟机磁盘文件，使用虚拟机磁盘文件新建虚拟机后直通 USB 设备后即可使用 One-KVM。下面以 Vmare 为例说明 One-KVM 虚拟机整合包的详细使用方法，VirtualBox 的操作方法大同小异。
...
在浏览器访问虚拟机 IP:8080 即可进入 WEB 界面。完成初始化引导流程后就可以开始使用。
